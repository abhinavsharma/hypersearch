...
class Bookmarks {
  ...
  /**
   * Convenience method to store IDs of newly created bookmarks.
   */
  private storeToAdd() {
  ...
  /**
   * Convenience method to store IDs of recently updated bookmarks.
   */
  ...
  /**
   * Convenience method to store IDs of recently deleted bookmarks.
   */
  ...
  /**
   * Convenience method to store remote to local IDs mapping.
   */
  ...
  private configureListeners() {
    ...
    const onRemove = (bookmarkId: string, node: chrome.bookmarks.BookmarkTreeNode) => {
      if (
        !(NOT_SYNCED as readonly string[]).includes(bookmarkId) &&
        !(NOT_SYNCED as readonly string[]).includes(this.getRemoteId(bookmarkId))
      ) {
   ...
   * @param apiToken token to access server API.
   * @param changes array of addeded or updated bookmarks.
  ...
  private updateRemoteBookmarks(remoteBookmarks: RemoteBookmark[]) {
    const promises = remoteBookmarks
      .map(async (remoteBookmark) => {
        ...
        if (!localId) {
          // Try to find local with same title/url.
  ...
  private async checkNewBookmarks(): Promise<void> {
    ...
    const check = (node: chrome.bookmarks.BookmarkTreeNode) => {
      ...
      if (
        !(NOT_SYNCED as readonly string[]).includes(node.id) &&
        !mappedLocalIds.includes(node.id)
      ) {
  ...
  private async uploadLocalChanges(apiToken: string) {
    ...
    const promises: Promise<any>[] = localChanges.map((localBookmark) => {
      return new Promise((resolve) => {
        chrome.bookmarks.get(localBookmark, (bookmarks) => {
          ...
          if (
            (NOT_SYNCED as readonly string[]).includes(bookmark.id) ||
            (NOT_SYNCED as readonly string[]).includes(this.getRemoteId(bookmark.id))
          ) {
            return resolve(null);
          }
          ...
          const remoteBookmark = {
            ...
            url: bookmark.url,
            type: bookmark.url ? BOOKMARK_TYPE.BOOKMARK : BOOKMARK_TYPE.FOLDER,
          } as RemoteBookmark;
    ...
    const deletions = this._toDelete
      .filter(
        (id) =>
          !(NOT_SYNCED as readonly string[]).includes(id) &&
          !(NOT_SYNCED as readonly string[]).includes(this.getRemoteId(id)),
      )
  ...
  private deleteLocalBookmark(localId: string, remoteBookmark: RemoteBookmark): Promise<void> {
    return new Promise((resolve) => {
      if (remoteBookmark.type === BOOKMARK_TYPE.FOLDER) {
  ...
  private groupParentsAndChildren(remoteBookmarks: RemoteBookmark[]) {
    ...
    const add = (bookmark: RemoteBookmark, parentId: string) => {
      if (bookmark.type !== BOOKMARK_TYPE.BOOKMARK && bookmark.type !== BOOKMARK_TYPE.FOLDER) {
        return;
      }
    ...
    remoteBookmarks.forEach((bookmark) => {
      ...
      if (bookmark.type === BOOKMARK_TYPE.FOLDER) {
