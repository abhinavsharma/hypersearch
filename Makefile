...
ship-is:
	...
	sed -i '' -e "s/IS_DEBUG_SWITCH = true/IS_DEBUG_SWITCH = false/g" ./src/utils/constants.ts
	npm run build
	zip -r `date +'INSIGHT-%Y-%m-%d-%H-%M'.zip` dist_is/
	mv `date +'INSIGHT-%Y-%m-%d-%H-%M'.zip` releases/
	sed -i '' -e "s/IS_DEBUG_SWITCH = false/IS_DEBUG_SWITCH = true/g" ./src/utils/constants.ts
	git add releases/*
