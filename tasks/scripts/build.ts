...
import webpack, { Compiler, StatsChunkGroup, StatsCompilation } from 'webpack';
import chalk from 'chalk';
import config from '../config/webpack.prod';

(async () => {
  (webpack(config()) as Compiler).run((err, stats) => {
    if (err?.message) {
    ...
    (Object.values(statData.namedChunkGroups ?? Object.create(null)) as StatsChunkGroup[]).forEach(
      (entry) => {
        entry.name &&
          console.log(
